...
def item_card(item: Item) -> str:
    ...
    original = "".join(photo_cell(p, alt=f"{item.code} original evidence", at=item.created_at) for p in item.original_photos) or empty_evidence("original evidence")
    rectification = "".join(
...
def build_report_html(items: list[Item], settings: Settings, report_type: str = "handover", *, subcontractor: str | None = None) -> str:
    ...
    project_config = settings.project_configs.get(settings.active_project)
    address = project_config.address if project_config else ""
    heading = "All Active Projects" if report_type == "subcontractor" and not settings.active_project else settings.active_project
    if settings.active_project and address:
        heading = f"{settings.active_project} - {address}"
    if subcontractor:
        title = f"{title} - {subcontractor}"
    share_filename = f"CleanRun-IQ-{report_type}-{date.today().isoformat()}.html"
