(function(){
  ...
  window.cardAction=function(event,id,act){event.preventDefault();event.stopPropagation();const button=event.currentTarget;if(button?.disabled)return false;button.disabled=true;button.classList.add("is-busy");itemAction(id,act).finally?.(()=>{button.disabled=false;button.classList.remove("is-busy")});return false};
  itemCard=function(i){
