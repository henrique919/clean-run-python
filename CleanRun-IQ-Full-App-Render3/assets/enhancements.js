(function(){
  ...
  const FOCUS_MODES=[["project","Active project"],["all","All projects"],["building","Buildings"],["level","Levels"],["unit","Units / areas"],["room","Rooms / locations"],["trade","Trades"],["subcontractor","Subcontractors"]];
  let itemFocusMode="project",itemFocusValue="";
  function focusLabel(mode){return (FOCUS_MODES.find(([value])=>value===mode)||FOCUS_MODES[0])[1]}
  function focusValues(mode){const cfg=state.settings.projectConfigs?.[state.settings.activeProject]||{};if(mode==="project")return state.settings.projects||[];if(mode==="building")return cfg.buildings||[];if(mode==="level")return cfg.levels||[];if(mode==="unit")return cfg.units||[];if(mode==="room")return cfg.rooms||[];if(mode==="trade")return trades||[];if(mode==="subcontractor")return state.settings.subcontractors||[];return []}
  function itemFocusControls(){const values=focusValues(itemFocusMode),valueSelect=values.length&&itemFocusMode!=="all"?`<select id="itemFocusValue" onchange="setItemFocusValue(this.value)"><option value="">All ${focusLabel(itemFocusMode).toLowerCase()}</option>${values.map(v=>`<option value="${esc(v)}" ${v===itemFocusValue?"selected":""}>${esc(v)}</option>`).join("")}</select>`:"";return `<div class="focus-controls"><select id="itemFocusMode" onchange="setItemFocusMode(this.value)">${FOCUS_MODES.map(([value,label])=>`<option value="${value}" ${value===itemFocusMode?"selected":""}>${label}</option>`).join("")}</select>${valueSelect}</div>`}
  window.setItemFocusMode=function(mode){itemFocusMode=mode;itemFocusValue="";render()};
  window.setItemFocusValue=function(value){itemFocusValue=value;filterItems()};
  const focusedItemsView=itemsView;
  itemsView=function(){const cfg=state.settings.projectConfigs?.[state.settings.activeProject]||{};itemFocusMode=cfg.itemsFocusMode||itemFocusMode||"project";const html=focusedItemsView();return html.replace("</div></div><div class=\"screen-scroll\">",`</div>${itemFocusControls()}</div><div class="screen-scroll">`)};
  filterItems=function(){const search=$("#search");if(!search)return;const q=search.value.toLowerCase();const list=state.items.filter(i=>{const inProject=itemFocusMode==="all"||i.project===state.settings.activeProject;const focusMatch=itemFocusMode==="all"||!itemFocusValue||String(i[itemFocusMode]||"")===itemFocusValue||(itemFocusMode==="project"&&i.project===itemFocusValue);return inProject&&focusMatch&&(itemTypeFilter==="all"||i.type===itemTypeFilter)&&statusMatch(i,itemStatusFilter)&&JSON.stringify(i).toLowerCase().includes(q)}).sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt));$("#itemCount").textContent=`${list.length} item${list.length===1?"":"s"}`;$("#itemList").innerHTML=list.map(itemCard).join("")||'<div class="empty">No items match<br><span class="meta">Try a different filter, focus area or capture a new item.</span></div>'};
  ...
  setupView=function(){const html=setupWithFocus(),cfg=state.settings.projectConfigs[state.settings.activeProject]||{},mode=cfg.itemsFocusMode||"project";const card=`<section class="form-card"><h2>Items page focus</h2><p class="meta">Choose the default way the Items page narrows attention for this project.</p><label>Default focus<select onchange="saveItemsFocusMode(this.value)">${FOCUS_MODES.map(([value,label])=>`<option value="${value}" ${value===mode?"selected":""}>${label}</option>`).join("")}</select></label></section>`;return html.replace("</section>",`</section>${card}`)};
  ...
  itemCard=function(i){const status=siteStatus(i),closed=["closed","complete"].includes(i.status),urgent=i.priority==="urgent";const dateText=closed?"CLOSED":`DUE ${esc(new Date(i.dueDate+"T00:00:00").toLocaleDateString(undefined,{day:"numeric",month:"short",year:"numeric"})).toUpperCase()}`;const location=[i.building,i.level,i.unit,i.room].filter(Boolean).join(" - ");const issueButton=i.status==="open"?`<button class="cr-issue-cta" type="button" onpointerdown="event.stopPropagation()" onclick="return cardAction(event,'${i.id}','issue')">ISSUE</button>`:"";const reissue=i.status==="rejected"?`<button class="cr-card-action" type="button" onpointerdown="event.stopPropagation()" onclick="return cardAction(event,'${i.id}','issue')">RE-ISSUE</button>`:"";return `<article class="native-card native-item cr-item-card status-${status.tone}" onclick="showItem('${i.id}')"><div class="cr-card-band"><div><span class="cr-card-code">${esc(i.code).toUpperCase()}</span><span class="cr-card-type">${esc(typeLabels[i.type]||i.type).toUpperCase()}</span></div><span class="cr-card-status badge ${status.tone}">${status.label}</span></div><div class="cr-card-main"><div class="cr-card-media">${cardPhoto(i)}<div class="cr-card-date under-photo">${dateText}</div></div><div class="cr-card-copy"><div class="cr-card-location">${esc(location||"NO LOCATION").toUpperCase()}</div><div class="cr-card-desc">${esc(i.description||"No description")}</div><div class="cr-card-assignment"><div class="cr-card-sub">${esc(i.subcontractor||"Unassigned")}</div><div class="cr-card-trade">${esc(i.trade||"No trade")}</div></div></div></div><div class="cr-card-actions" onclick="event.stopPropagation()">${issueButton}${reissue}${urgent?'<span class="badge overdue">URGENT</span>':""}</div></article>`};
