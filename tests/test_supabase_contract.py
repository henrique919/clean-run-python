...
class SupabaseContractTests(unittest.TestCase):
    ...
    def test_item_photos_sync_uses_upsert_for_markup_replacements(self) -> None:
        store = (ROOT / "app/store_supabase.py").read_text(encoding="utf-8")

        self.assertIn("def _sync_item_photos", store)
        self.assertIn('table("item_photos").upsert', store)
